...
def SSD(left_image, right_image, min_depth, max_depth, kernel_size, disparity_print):
                ...
                for h in range(height):
                    ...
                    for (i,j) in kernel_vectors: 
                        if h+i < height and w+d+j < width and h+i >= 0 and  w+j >= 0:
                            flag_sum += np.square(left_image[h+i,w+j] - right_image[h+i,w+d+j])
                            kernel_num += 1
                ...
                for h in range(height):
                    ...
                    for (i,j) in kernel_vectors: 
                        if h+i < height and w+j < width and h+i >= 0 and  w-d+j >= 0:
                            flag_sum += np.square(left_image[h+i,w-d+j] - right_image[h+i,w+j])
                            kernel_num += 1
